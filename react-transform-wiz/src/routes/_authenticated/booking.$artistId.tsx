...
import { z } from "zod";
import { CreditCard, Smartphone, Wallet, QrCode, Lock, CheckCircle2, Home, Building2 } from "lucide-react";

const searchSchema = z.object({
  services: z.string().optional().default(""),
  mode: z.enum(["at_home", "studio"]).optional(),
});

export const Route = createFileRoute("/_authenticated/booking/$artistId")({
...
function BookingPage() {
  ...
  const { services: serviceCsv, mode } = Route.useSearch();
  ...
  const canProceed = !!slotId && !!name && !!phone && (locationType === "studio" || !!address);
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="mx-auto max-w-5xl px-4 py-8">
        ...
        <ol className="mb-6 flex flex-wrap items-center gap-2 text-xs">
          {[
            { n: 1, label: "Service mode" },
            { n: 2, label: "Date & time" },
            { n: 3, label: "Details" },
            { n: 4, label: "Payment" },
          ].map((s, i) => {
            const active = (step === 1 && s.n <= 3) || (step === 2 && s.n <= 4);
            const done = (step === 1 && s.n < 3) || (step === 2 && s.n < 4);
            return (
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              ...
              <span className="inline-flex items-center gap-1 text-foreground">
                {locationType === "studio" ? <Building2 className="h-3.5 w-3.5" /> : <Home className="h-3.5 w-3.5" />}
                {locationType === "studio" ? "Studio service" : "Home service"}
              </span>
            </p>
                ...
                {locationType === "at_home" && (
                  <section className="rounded-xl border border-border bg-card p-4">
                    ...
                    <div className="mt-3 grid gap-2 sm:grid-cols-[1fr_180px]">
                      <input value={address} onChange={(e) => setAddress(e.target.value)} className="rounded-md border border-input bg-background px-3 py-2 text-sm" placeholder="Full address" />
                      <input value={city} onChange={(e) => setCity(e.target.value)} className="rounded-md border border-input bg-background px-3 py-2 text-sm" placeholder="City" />
                    </div>
                  </section>
                )}
            ...
            {step === 2 && (
              <div className="mt-6 space-y-6">
                ...
                <section className="rounded-xl border border-amber-500/30 bg-gradient-to-br from-amber-50/40 to-transparent p-4 dark:from-amber-950/20">
                  <h2 className="font-medium">Choose payment plan</h2>
                  <div className="mt-3 grid gap-3 sm:grid-cols-2">
                    <button type="button" onClick={() => setIsAdvance(false)} className={`rounded-lg border p-4 text-left ${!isAdvance ? "border-primary ring-2 ring-primary/30 bg-primary/5" : "border-border"}`}>
                      <div className="text-xs uppercase tracking-wider text-muted-foreground">Full payment</div>
                      <div className="mt-1 text-2xl font-semibold">{formatINR(total)}</div>
                      <div className="mt-1 text-xs text-muted-foreground">Pay 100% now · No balance later</div>
                    </button>
                    <button type="button" onClick={() => setIsAdvance(true)} className={`rounded-lg border p-4 text-left ${isAdvance ? "border-primary ring-2 ring-primary/30 bg-primary/5" : "border-border"}`}>
                      <div className="text-xs uppercase tracking-wider text-muted-foreground">Pay 25% advance</div>
                      <div className="mt-1 text-2xl font-semibold">{formatINR(advanceAmount)}</div>
                      <div className="mt-1 text-xs text-muted-foreground">Balance {formatINR(total - advanceAmount)} after service</div>
                    </button>
                  </div>
                </section>
                ...
                <section className="rounded-xl border border-border bg-card p-4">
                  ...
                  <div className="mt-3 grid grid-cols-3 gap-2">
                    {[
                      ...
                      { id: "wallet" as const, label: "Wallet", icon: <Wallet className="h-4 w-4" /> },
                    ].map((m) => (
                      <button key={m.id} type="button" onClick={() => setPayMethod(m.id)} className={`flex items-center justify-center gap-2 rounded-md border px-3 py-2 text-sm ${payMethod === m.id ? "border-primary bg-primary/5 font-medium" : "border-border hover:bg-accent"}`}>
                        {m.icon} {m.label}
                      </button>
                    ))}
                  </div>
          ...
          <aside className="lg:sticky lg:top-20 lg:self-start">
            <div className="rounded-2xl border border-border bg-card p-5">
              ...
              <div className="mt-3 space-y-2 text-sm">
                ...
                <Row label="Mode" value={<span className="inline-flex items-center gap-1">{locationType === "studio" ? <Building2 className="h-3 w-3" /> : <Home className="h-3 w-3" />} {locationType === "studio" ? "Studio" : "Home"}</span>} />
              ...
              <div className="space-y-1.5 text-sm">
                ...
                {isAdvance && (
                  <>
                    <div className="flex justify-between text-emerald-600"><span>Advance (25%)</span><span>{formatINR(advanceAmount)}</span></div>
